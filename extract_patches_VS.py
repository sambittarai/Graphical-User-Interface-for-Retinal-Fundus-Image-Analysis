...
def load_data(test_img_path):
    ...
    '''
    # img = np.asarray(Image.open(test_img_path)) # Uncomment it out if high resolution images are not being used
    img = Image.open(test_img_path) # Comment it out if high resolution images are not being used
    img = img.resize((int(img.size[0]/2), int(img.size[1]/2))) # Comment it out if high resolution images are not being used
    img = np.asarray(img) # Comment it out if high resolution images are not being used
...
def load_data_preprocess(test_img_path):
    '''
    Load the original image, grroundtruth and FOV of the data set in order, and check the dimensions
    # This function is up to date and working fine for the GUI 
    '''
...
# =============================Load test data==========================================
def get_data_test_overlap_VS(test_img_path, patch_height, patch_width, stride_height, stride_width):
    '''
...
def recompone_overlap_VS(preds, img_h, img_w, stride_h, stride_w):
    '''
